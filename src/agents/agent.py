...
class TaskManager:
    ...
    def create(self, subject: str, description: str = "") -> str:
        task = {
            ...
            "status": "pending", "blockedBy": [], "owner": "", "worktree": "",
...
class WorktreeManager:
    def __init__(self, repo_root: Path, store_dir: Path, tasks: TaskManager, events: EventBus):
    ...
    def create(self, name: str, task_id: int | None = None, base_ref: str = "HEAD") -> str:
        ...
        try:
            ...
            self.events.emit(
                ...
                task={"id": task_id} if task_id is not None else {},
                worktree={"name": name, "path": str(wt_path), "branch": branch, "status": "active"},
            )
    ...
    def list_all(self) -> str:
        ...
        lines = []
        for wt in wts:
            suffix = f" task={wt['task_id']}" if wt.get("task_id") is not None else ""
            lines.append(
                f"[{wt.get('status', 'unknown')}] {wt.get('name')} -> {wt.get('path')} ({wt.get('branch', '-')}){suffix}"
            )
        return "\n".join(lines)
    ...
    def keep(self, name: str) -> str:
        ...
        self.events.emit(
            "worktree.keep",
            task={"id": kept.get("task_id")} if kept.get("task_id") is not None else {},
            worktree={"name": name, "path": kept.get("path"), "status": "kept"},
        )
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "owner": {
                    "type": "string",
                    "description": "Optional owner to set on task.",
                },
            },
...
},
{
    ...
    "function": {
        ...
        "description": "Create a git worktree and optionally bind to a task.",
        "parameters": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string",
                    "description": "Worktree lane name.",
                },
                "task_id": {
                    "type": "integer",
                ...
                "base_ref": {
                    "type": "string",
                    "description": "Git base ref (default HEAD).",
                },
            },
            "required": ["name"],
        },
...
},
{
    ...
    "function": {
        "name": "worktree_list",
        "description": "List tracked worktrees from index.",
        "parameters": {
            "type": "object",
        },
    },
...
},
{
    ...
    "function": {
        "name": "worktree_run",
        "description": "Run a shell command inside a named worktree.",
        "parameters": {
            "type": "object",
            "properties": {
                "name": {
...
},
{
    ...
    "function": {
        "name": "worktree_keep",
        "description": "Mark a worktree as kept (do not remove).",
        "parameters": {
            "type": "object",
            "properties": {
                "name": {
...
},
{
    ...
    "function": {
        ...
        "description": "Remove a worktree and optionally complete bound task.",
        "parameters": {
            "type": "object",
            "properties": {
                "name": {
                ...
                "force": {
                    "type": "boolean",
                    "description": "Force remove even with local changes.",
                },
                "complete_task": {
                    "type": "boolean",
                    "description": "If true, complete bound task and unbind.",
                },
            },
            "required": ["name"],
        },
...
},
{
    ...
    "function": {
        ...
        "description": "Read recent worktree/task lifecycle events.",
        "parameters": {
            "type": "object",
            "properties": {
                "limit": {
